...
async function downloadHandler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  ...
  try {
    ...
    const tokenBindIp = xForwardedFor ? xForwardedFor.split(',')[0].trim() : clientIp;
...
async function handleTokenDownload(
  ...
  try {
    ...
    return createErrorResponse(ErrorCode.STORAGE_ERROR, 'Failed to process download', origin);
  }
}
